...
클라우드AI프로그래밍 기말 프로젝트입니다. LangChain · LangGraph 멀티에이전트와 RAG로 구성하며, 채팅은 외부 LLM, 임베딩은 Gemini를 쓰는 하이브리드 구성입니다.

단톡방·SNS로 퍼지는 뉴스·루머를 텍스트 또는 캡처 이미지로 받아 검증 가능한 주장을 뽑고, 검사·변호 양측의 전속 리서처가 각자 관점(반박·지지)으로 근거를 수집한 뒤 검사 → 변호 반박 → 검사 재반박의 3턴 적대 토론과 판사의 자가 반박 루프를 거쳐 신뢰도 등급·근거 사슬·토론 전문·조작 기법 태그·반론 카드를 돌려주는 코치형 에이전트입니다.

단정적 라벨만 내놓는 분류기가 아니라 왜 헷갈리게 만드는지와 어떻게 반박할지까지 알려주어 사용자의 미디어 리터러시를 키우는 것을 목표로 합니다.
...
flowchart TD
    ...
    EX --> RE[retrieve_evidence<br/>양측 전속 리서처 RAG]
    EX --> TT[tag_techniques<br/>조작 기법 태깅 RAG · 병렬]
    RE --> AD[adversarial_debate<br/>검사 → 변호 → 검사 3턴]
    RE -->|재검색 신규 증거 없음| SY
...
- 멀티모달 입력: 텍스트뿐 아니라 카톡 캡처·SNS 짤 같은 이미지를 첨부하면 비전 LLM이 이미지 속 텍스트·맥락에서 주장을 직접 추출합니다(텍스트 경로는 무변경).
- 양측 전속 리서처: `retrieve_evidence`가 주장당 반박 관점·지지 관점 2개 쿼리로 검색해 스니펫에 스탠스(`prosecution`/`defense`/`both`)를 태깅합니다. 검사와 변호는 각자 자기 리서처가 모은 증거 풀만 인용합니다(기존 토론형 팩트체커는 증거를 양측 공유).
- 3턴 적대 토론: 검사 발언, 변호가 검사 논거를 읽고 반박, 검사 재반박 순서로 진행합니다. 토론 전문이 `debate_transcript`로 누적되어 웹 UI 대화창(검사·변호 채팅 버블)에 표시됩니다. 관련 코퍼스 증거가 0건인 주장(범위 밖 주제)은 일반 상식 토론으로 전환됩니다. 인용 없이 상식 수준 논거만 펴고(수치·출처 날조 금지), 모드 전환이 대화창에 안내됩니다.
- 멀티에이전트: 검사, 변호, 판사, 기법 태거, 반론 작성(코치)을 한 LLM에 역할별 시스템 프롬프트로 구현합니다.
- 자가 반박 루프: 판사가 증거 부족이라 판단하면 `retrieve_evidence`로 되돌아가며(이때 「추가 증거 수집 지시」 시스템 턴이 대화창에 표시됨), 4가지 핵심 종료 조건(최대 루프·판사 만족·신규 증거 없음·신뢰도 수렴)으로 무한루프·진동을 막습니다. 재검색에서 신규 증거가 0건이면 토론·판사를 다시 돌리지 않고 직전 판정으로 바로 합성합니다(재검색 쿼리는 주장 텍스트만의 함수라 결과가 동일하므로 호출 낭비를 차단).

## 2. 안전 설계 (환각·오판 방지)

- 검색 관련성 임계값은 코퍼스 실측으로 보정했습니다(주제 일치 0.717 이상, 무관 0.676 이하로 분리해 기본값 0.70). 코퍼스 밖 주장에는 무관 스니펫이 증거로 흘러들지 않고 「관련 증거 0건 → 일반 상식 토론 전환(인용 없음, 단정 판정 신뢰도 상한 0.85)」으로 정직하게 처리됩니다.
- 검사·변호는 실제 회수된 스니펫만 인용하도록 프롬프트로 강제하고, 코드 레벨에서 지어낸 인용 id를 제거합니다(이중 방어). 판사 판정도 같은 가드를 거치며, 인용 전부가 지어낸 id인 단정 판정은 「불충분」으로 강등됩니다.
- 각 증거에는 매칭도(주장-증거 유사도)와 작성시점이 표기되어 판사·토론자가 턱걸이 매칭과 오래된 근거를 구분합니다.
- 판사는 출처 없는·신뢰도 낮은 주장을 감점하고, 「불충분(판단 불가)」를 정식 결론으로 허용합니다(프롬프트 유도와 함께 「증거 부재는 거짓의 증거가 아니다」 규칙 적용). 코퍼스 인용 없이 일반 상식만으로 내린 단정 판정은 신뢰도 상한 0.85가 적용되고 결과에 「일반 상식 기반」으로 표기됩니다.
- 출력은 단정 라벨이 아니라 5단계 신뢰도 등급(사실, 대체로 사실, 불충분, 대체로 거짓, 거짓·오도)입니다. 주장별 판정이 참·거짓으로 엇갈리면 종합 등급은 평균으로 뭉개지 않고 「혼재(일부 사실·일부 거짓)」로 분리해 표시합니다.
- 최종 리포트의 핵심 수치(종합 등급·근거 출처)는 Python에서 결정론적으로 계산하고, LLM은 자연어 반론 카드만 작성합니다.

---

## 3. 설치 (조교/채점자 안내)

요구사항: Python 3.12 권장(3.11~3.13), 인터넷 연결, 그리고 키 2개가 필요합니다. 채팅용 외부 LLM API 키(`LLM_API_KEY`)와 임베딩용 Gemini 키(`GOOGLE_API_KEY`)입니다. 본 에이전트는 채팅은 외부 LLM, 임베딩(RAG)은 Gemini를 쓰는 하이브리드 구성입니다.
...
제출본의 `.env.example`에는 키가 `YOUR-API-KEY-HERE`로 비워져 있습니다. 실제 키는 `.env`에만 넣으며, `.env`는 `.gitignore`로 커밋되지 않습니다. 키가 없으면 프로그램은 스택트레이스 대신 친절한 한국어 안내 후 종료합니다.

모델·비용 안내. 이 에이전트는 한 번의 검증에 LLM을 여러 번 호출합니다(주장추출·3턴 토론(주장당 3회)·판사와 자가반박·기법태깅·반론으로 대략 7~12회, 루프 시 증가). 이미지 첨부 시 비전 입력 토큰이 추가됩니다(1MP가 약 1,300토큰). 비용은 `LLM_MODEL`로 지정한 모델 단가에 비례하므로, 비용에 민감하면 저비용·고속 모델 ID를 사용하세요. `LLM_MAX_TOKENS`·`RETRIEVE_K`·`MAX_CLAIMS`를 낮추면 호출·토큰이 줄어듭니다. 레이트리밋(429/529) 시 자동 백오프 재시도하며, 필요하면 `LLM_THROTTLE_SECONDS`로 호출 간격을 둘 수 있습니다. Gemini 임베딩 무료 등급은 일일 요청 한도가 있을 수 있으니, 대량 시연은 한도가 넉넉한 키에서 실행하세요.
...
```
인덱스(`data/.chroma/`)는 커밋하지 않습니다. 소스 JSON만 커밋하고 각 로컬에서 동일하게 재빌드되므로 어떤 환경에서도 같은 결과를 얻습니다.

---

## 4. 실행

설치 없이 바로 체험하려면 https://factchecker-nlvz.onrender.com/ 에 접속하세요(Render 상시 배포). BYOK 방식으로 화면 상단에 본인의 외부 LLM API 키를 입력하면 동작하며, 키는 서버에 저장하지 않습니다. 무료 등급이라 첫 접속 시 콜드 스타트로 수십 초가 걸릴 수 있습니다.
...
| `MAX_IMAGE_CHARS` | `1400000` | 첨부 이미지(data URL) 길이 상한(`server.py`) |
...
factchecker/            # 백엔드 패키지 (저장소 루트, flat layout)
  ...
  llm.py                # 외부 LLM·임베딩 팩토리 + 안전한 구조화 출력(멀티모달 지원)
  prompts/              # 역할별 프롬프트 템플릿(.txt 7종)
  rag/                  # 벡터스토어·인제스트·양측(스탠스) 증거/기법 회수
