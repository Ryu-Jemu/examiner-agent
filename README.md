...
단톡방·SNS로 퍼지는 뉴스·루머를 **텍스트 또는 캡처 이미지**로 받아 **검증 가능한
주장**을 뽑고, **검사·변호 양측의 전속 리서처**가 각자 관점(반박/지지)으로 근거를
수집한 뒤 **검사 → 변호 반박 → 검사 재반박의 3턴 적대 토론**과 **판사의 자가 반박
루프**를 거쳐 **신뢰도 등급 · 근거 사슬 · 토론 전문 · 조작 기법 태그 · 반론 카드**를
돌려주는 코치형 에이전트입니다.
...
- **멀티모달 입력**: 텍스트뿐 아니라 카톡 캡처·SNS 짤 같은 **이미지**를 첨부하면 비전
  LLM이 이미지 속 텍스트·맥락에서 주장을 직접 추출합니다(텍스트 경로는 무변경).
- **양측 전속 리서처**: `retrieve_evidence`가 주장당 **반박 관점·지지 관점 2개 쿼리**로
  검색해 스니펫에 스탠스(`prosecution`/`defense`/`both`)를 태깅 — 검사와 변호는 각자
  자기 리서처가 모은 증거 풀만 인용합니다(기존 토론형 팩트체커는 증거를 양측 공유).
- **3턴 적대 토론**: 검사 발언 → 변호가 검사 논거를 읽고 반박 → 검사 재반박. 토론
  전문이 `debate_transcript`로 누적되어 웹 UI 대화창(검사·변호 채팅 버블)에 표시됩니다.
- **멀티에이전트**: 검사 / 변호 / 판사 / 기법 태거 / 반론 작성(코치) — 한 LLM에 역할별 시스템 프롬프트로 구현.
- **자가 반박 루프**: 판사가 증거 부족이라 판단하면 `retrieve_evidence`로 되돌아가며(이때 "추가 증거 수집 지시" 시스템 턴이 대화창에 표시됨), 4가지 핵심 종료 조건(최대 루프·판사 만족·신규 증거 없음·신뢰도 수렴)으로 무한루프/진동을 막습니다. 재검색에서 신규 증거가 0건이면 토론·판사를 다시 돌리지 않고 직전 판정으로 바로 합성합니다(재검색 쿼리는 주장 텍스트만의 함수라 결과가 동일 — 호출 낭비 차단).

## 2. 안전 설계 (환각·오판 방지)

- 검색 관련성 임계값은 **코퍼스 실측으로 보정**했습니다(주제 일치 0.717 이상 / 무관 0.676 이하로 분리 → 기본값 0.70). 코퍼스 밖 주장에는 무관 스니펫이 증거로 흘러들지 않고 "관련 증거 0건 → 토론 생략 → 불충분"으로 정직하게 떨어집니다.
- 검사·변호는 **실제 회수된 스니펫만 인용**하도록 프롬프트로 강제하고, **코드 레벨에서 지어낸 인용 id를 제거**합니다(이중 방어). 판사 판정도 같은 가드를 거치며, **인용 전부가 지어낸 id인 단정 판정은 "불충분"으로 강등**됩니다.
- 각 증거에는 **매칭도(주장-증거 유사도)와 작성시점**이 표기되어 판사·토론자가 턱걸이 매칭과 오래된 근거를 구분합니다.
- 판사는 출처 없는·신뢰도 낮은 주장을 **감점**하고, **"불충분(판단 불가)"** 를 정식 결론으로 허용합니다(프롬프트 유도 + "증거 부재는 거짓의 증거가 아니다" 규칙). 코퍼스 인용 없이 일반 상식만으로 내린 단정 판정은 **신뢰도 상한 0.85**가 적용되고 결과에 "일반 상식 기반"으로 표기됩니다.
...
> (주장추출·3턴 토론(주장당 3회)·판사+자가반박·기법태깅·반론, 대략 7~12회, 루프 시
> 증가). 이미지 첨부 시 비전 입력 토큰이 추가됩니다(1MP ≈ 1,300토큰). 비용은 `LLM_MODEL` 로
...
> 💡 **채점 전 워밍업(권장):** 무료 등급은 미접속 시 서버가 잠들어 첫 접속이 콜드
> 스타트로 수십 초 걸릴 수 있습니다. 시연 직전에 `curl https://factchecker-nlvz.onrender.com/health`
> 를 한 번 호출해 깨워 두세요.
